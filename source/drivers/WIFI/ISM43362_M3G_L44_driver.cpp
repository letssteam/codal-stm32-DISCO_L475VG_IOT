            ...
            if ((*curr_ptr) != '#') {
                ...
                if (AT_ParseSingleAP(pdata, &(ESWifiApObj.AP[ESWifiApObj.nbr]))) {
                    ...
                    if (ret == ES_WIFI_STATUS_REQ_DATA_STAGE) {
                        ...
                        ret = AT_ReceiveCommand(EsWifiObj.CmdData + strlen(curr_ptr),
                                                ES_WIFI_DATA_SIZE - strlen(curr_ptr));
                        if ((ret != ES_WIFI_STATUS_OK) && (ret != ES_WIFI_STATUS_REQ_DATA_STAGE)) {
                            break;
                        }
                        curr_ptr = strtok_r((char*)(EsWifiObj.CmdData), "\n", &sav_ptr);
...
ES_WIFI_Status_t IsmDrvClass::ES_WIFI_Connect(const char* SSID, const char* Password, ES_WIFI_SecurityType_t SecType)
{
    ...
    sprintf((char*)EsWifiObj.CmdData, "%s=%s%s", AT_NET_SET_SSID, SSID, SUFFIX_CMD);
    ret = AT_ExecuteCommand();
    if (ret == ES_WIFI_STATUS_OK) {
        sprintf((char*)EsWifiObj.CmdData, "%s=%s%s", AT_NET_SET_PASSPHRASE, Password, SUFFIX_CMD);
        ret = AT_ExecuteCommand();

        if (ret == ES_WIFI_STATUS_OK) {
            EsWifiObj.Security = SecType;
            sprintf((char*)EsWifiObj.CmdData, "%s=%d%s", AT_NET_SET_SECURITY_TYPE, SecType, SUFFIX_CMD);
            ret = AT_ExecuteCommand();
...
void IsmDrvClass::ES_WIFI_SetMACAddress(uint8_t* mac)
{
    ...
    sprintf((char*)EsWifiObj.CmdData, "%s=%X:%X:%X:%X:%X:%X%s", AT_SYS_SET_MAC_ADDR, mac[0], mac[1], mac[2], mac[3],
            mac[4], mac[5], SUFFIX_CMD);
...
void IsmDrvClass::ES_WIFI_SetProductName(uint8_t* ProductName)
{
    ...
    sprintf((char*)EsWifiObj.CmdData, "%s=%s%s", AT_SYS_SET_PRODUCT_NAME, ProductName, SUFFIX_CMD);
...
void IsmDrvClass::ES_WIFI_Ping(uint8_t* address, uint16_t count, uint16_t interval_ms)
{
    ...
    sprintf((char*)EsWifiObj.CmdData, "%s=%d.%d.%d.%d%s", AT_PING_SET_TARGET_ADDR, address[0], address[1], address[2],
            address[3], SUFFIX_CMD);

    ret = AT_ExecuteCommand();

    if (ret == ES_WIFI_STATUS_OK) {
        sprintf((char*)EsWifiObj.CmdData, "%s=%d%s", AT_PING_SET_REPEAT_COUNT, count, SUFFIX_CMD);
        ret = AT_ExecuteCommand();

        if (ret == ES_WIFI_STATUS_OK) {
            sprintf((char*)EsWifiObj.CmdData, "%s=%d%s", AT_PING_SET_DELAY_MS, interval_ms, SUFFIX_CMD);
            ret = AT_ExecuteCommand();
...
void IsmDrvClass::ES_WIFI_DNS_LookUp(const char* url, IPAddress* ipaddress)
{
    ...
    sprintf((char*)EsWifiObj.CmdData, "%s=%s%s", AT_DNS_LOOKUP, url, SUFFIX_CMD);
    ret = AT_ExecuteCommand();
...
void IsmDrvClass::ES_WIFI_StartClientConnection(uint8_t index)
{
    ...
    sprintf((char*)EsWifiObj.CmdData, "%s=%d%s", AT_TR_SET_SOCKET, ESWifiConnTab[index].Number, SUFFIX_CMD);
    ret = AT_ExecuteCommand();

    if (ret == ES_WIFI_STATUS_OK) {
        sprintf((char*)EsWifiObj.CmdData, "%s=%d%s", AT_TR_SET_PROTOCOL, ESWifiConnTab[index].Type, SUFFIX_CMD);
        ret = AT_ExecuteCommand();
        if (ret == ES_WIFI_STATUS_OK) {
            if ((ESWifiConnTab[index].Type == ES_WIFI_UDP_CONNECTION) && (ESWifiConnTab[index].LocalPort > 0)) {
                sprintf((char*)EsWifiObj.CmdData, "%s=%d%s", AT_TR_SET_LOCAL_PORT_NUMBER,
                        ESWifiConnTab[index].LocalPort, SUFFIX_CMD);
                ret = AT_ExecuteCommand();
            }
            sprintf((char*)EsWifiObj.CmdData, "%s=%d%s", AT_TR_SET_REMOTE_PORT_NUMBER, ESWifiConnTab[index].RemotePort,
                    SUFFIX_CMD);

            ret = AT_ExecuteCommand();

            if (ret == ES_WIFI_STATUS_OK) {
                sprintf((char*)EsWifiObj.CmdData, "%s=%d.%d.%d.%d%s", AT_TR_SET_REMOTE_HOST_IP_ADDR,
                        ESWifiConnTab[index].RemoteIP[0], ESWifiConnTab[index].RemoteIP[1],
                        ESWifiConnTab[index].RemoteIP[2], ESWifiConnTab[index].RemoteIP[3], SUFFIX_CMD);
                ret = AT_ExecuteCommand();

                if (ret == ES_WIFI_STATUS_OK) {
                    sprintf((char*)EsWifiObj.CmdData, "%s=1%s", AT_TR_CLIENT, SUFFIX_CMD);
                    ret = AT_ExecuteCommand();
                }
            }
        }
...
void IsmDrvClass::ES_WIFI_StopClientConnection(uint8_t index)
{
    ...
    sprintf((char*)EsWifiObj.CmdData, "%s=%d%s", AT_TR_SET_SOCKET, index, SUFFIX_CMD);
    ret = AT_ExecuteCommand();

    if (ret == ES_WIFI_STATUS_OK) {
        sprintf((char*)EsWifiObj.CmdData, "%s=0%s", AT_TR_CLIENT, SUFFIX_CMD);
        ret = AT_ExecuteCommand();
    }
}
...
void IsmDrvClass::ES_WIFI_StartServerSingleConn(uint8_t index, comm_mode mode)
{
    ...
    sprintf((char*)EsWifiObj.CmdData, "%s=1,3000%s", AT_TR_TCP_KEEP_ALIVE, SUFFIX_CMD);
    ret = AT_ExecuteCommand();
    if (ret == ES_WIFI_STATUS_OK) {
        currentSock            = index;
        ...
        if (ret == ES_WIFI_STATUS_OK) {
            ...
            if (ret == ES_WIFI_STATUS_OK) {
                ...
                ret = AT_ExecuteCommand();
                if (ret == ES_WIFI_STATUS_OK) {
                    sprintf((char*)EsWifiObj.CmdData, "%s=1%s", AT_TR_SERVER, SUFFIX_CMD);
                    ret = AT_ExecuteCommand();
                    ...
                    if (ret == ES_WIFI_STATUS_OK) {
                        if (mode == COMM_UART) {
                            ...
                            do {
                                ...
                                ret = AT_ExecuteCommand();
                                if (ret == ES_WIFI_STATUS_OK) {
                                    if ((strstr((char*)EsWifiObj.CmdData, "[SOMA]")) &&
                                        (strstr((char*)EsWifiObj.CmdData, "[EOMA]"))) {
                                        if (strstr((char*)EsWifiObj.CmdData, "Accepted")) {
                                            ptr = strtok((char*)EsWifiObj.CmdData + 2, " ");
                                            ptr = strtok(NULL, " ");
                                            ptr = strtok(NULL, " ");
...
void IsmDrvClass::ES_WIFI_StopServerSingleConn(uint8_t index)
{
    ...
    ret = AT_ExecuteCommand();
    if (ret == ES_WIFI_STATUS_OK) {
        sprintf((char*)EsWifiObj.CmdData, "%s=0%s", AT_TR_SERVER, SUFFIX_CMD);
        AT_ExecuteCommand();
    }
}
    ...
    if (ret == ES_WIFI_STATUS_OK) {
        ...
        if (ret == ES_WIFI_STATUS_OK) {
            ...
            if (ret == ES_WIFI_STATUS_OK) {
                ...
                ret = AT_ExecuteCommand();
                if (ret == ES_WIFI_STATUS_OK) {
                    sprintf((char*)EsWifiObj.CmdData, "%s=6%s", AT_TR_SET_LISTEN_BACKLOGS, SUFFIX_CMD);
                    ret = AT_ExecuteCommand();
                    ...
                    if (ret == ES_WIFI_STATUS_OK) {
                        ...
                        if (ret == ES_WIFI_STATUS_OK) {
                            sprintf((char*)EsWifiObj.CmdData, "%s=1%s", AT_TR_REQUEST_TCP_LOOP, SUFFIX_CMD);
                            ret = AT_ExecuteCommand();
                        }
                    }
                }
...
void IsmDrvClass::ES_WIFI_StopServerMultiConn()
{
    ...
    sprintf((char*)EsWifiObj.CmdData, "%s=2%s", AT_TR_REQUEST_TCP_LOOP, SUFFIX_CMD);
    ret = AT_ExecuteCommand();

    if (ret == ES_WIFI_STATUS_OK) {
        /*Get the next request out of the queue */
        sprintf((char*)EsWifiObj.CmdData, "%s=3%s", AT_TR_REQUEST_TCP_LOOP, SUFFIX_CMD);
        ret = AT_ExecuteCommand();
        if (ret == ES_WIFI_STATUS_OK) {
            if (ret == ES_WIFI_STATUS_OK) {
                if (Drv->IO_Receive(EsWifiObj.CmdData, 0, EsWifiObj.Timeout) > 0) {
...
void IsmDrvClass::ES_WIFI_SendResp(uint8_t Socket, uint8_t* pdata, uint16_t Reqlen, uint16_t* SentLen, uint32_t Timeout)
{
    ...
    if (Reqlen <= ES_WIFI_PAYLOAD_SIZE) {
        currentSock = Socket;
        sprintf((char*)EsWifiObj.CmdData, "%s=%d%s", AT_TR_SET_SOCKET, Socket, SUFFIX_CMD);
        ret = AT_ExecuteCommand();
        if (ret == ES_WIFI_STATUS_OK) {
            ES_WIFI_SetTimeout(Timeout);
            // Timeout for the device must be shorter than our timeout otherwise
            // the device may answer to late.
            sprintf((char*)EsWifiObj.CmdData, "%s=%lu%s", AT_WRITE_SET_TIMEOUT, Timeout - TIMEOUT_OFFSET, SUFFIX_CMD);
            ret = AT_ExecuteCommand();
            if (ret == ES_WIFI_STATUS_OK) {
                sprintf((char*)EsWifiObj.CmdData, "%s=%u%s", AT_WRITE_SET_PACKET_SIZE, Reqlen, SUFFIX_CMD);
                ret = AT_ExecuteCommand();
                if (ret == ES_WIFI_STATUS_OK) {
                                      ...
                                      uint32_t Timeout)
{
    ...
    if (Reqlen <= ES_WIFI_PAYLOAD_SIZE) {
        ...
        if (ret == ES_WIFI_STATUS_OK) {
            sprintf((char*)EsWifiObj.CmdData, "%s=%d%s", AT_READ_SET_PACKET_SIZE, Reqlen, SUFFIX_CMD);
            ret = AT_ExecuteCommand();
            if (ret == ES_WIFI_STATUS_OK) {
                // Timeout for the device must be shorter than our timeout otherwise
                // the device may answer too late.
                ES_WIFI_SetTimeout(Timeout);
                sprintf((char*)EsWifiObj.CmdData, "%s=%lu%s", AT_READ_SET_TIMEOUT_MS, Timeout - TIMEOUT_OFFSET,
                        SUFFIX_CMD);
                ret = AT_ExecuteCommand();
                if (ret == ES_WIFI_STATUS_OK) {
